...
def main():
    ...
    for path in DIST.rglob("*.html"):
        ...
        page = re.sub(r'/assets/css/style\.css(?:\?v=\d+)?', '/assets/css/style.css?v=29', page)
        page = re.sub(r'/assets/js/app\.js(?:\?v=\d+)?', '/assets/js/app.js?v=29', page)
        if '/assets/js/nav.js' not in page:
            page = page.replace('</body>', '<script src="/assets/js/nav.js?v=29" defer></script>\n</body>')
        path.write_text(page, encoding="utf-8")
